...
import { fetchNonVegProducts } from "./Store";  // You need to create this thunk/action in your store
import { addToCart } from "./Store";
import { toast } from "react-toastify";

function NonVeg() {
  const dispatch = useDispatch();
  const { nonVegProducts, loading, error } = useSelector((state) => state.nonVeg); // Make sure nonVeg slice exists
  ...
  useEffect(() => {
    dispatch(fetchNonVegProducts()); // Call MongoDB API for non-veg products
  }, [dispatch]);

  if (loading) return <h2>Loading non-veg products...</h2>;
  if (error) return <h2 style={{ color: "red" }}>{error}</h2>;
  if (!nonVegProducts || nonVegProducts.length === 0)
    return <h2>No non-veg products found in MongoDB</h2>;

  // Pagination Logic
  const totalPages = Math.ceil(nonVegProducts.length / itemsPerPage);
  const indexOfLast = currentPage * itemsPerPage;
  const indexOfFirst = indexOfLast - itemsPerPage;
  const currentItems = nonVegProducts.slice(indexOfFirst, indexOfLast);

  return (
    <div className="veg-container">
      <h1 className="veg-heading">Welcome to Non-Veg Section</h1>
